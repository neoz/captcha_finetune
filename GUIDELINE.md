...
Heavy augmentation to compensate for the small dataset (400 train / 100 test):
...
|---|---|---|
| Full captcha accuracy | 95.0% | **99.0%** |
| Per-character accuracy | 96.4% | **99.8%** |
| Errors on test set | 5 | **1** |
| Model parameters | 1,450,721 | **1,328,352** |
| Model size | 5.6 MB | **5.1 MB** |

The single remaining error: `AW79Q` predicted as `AWY9Q` — a genuine visual ambiguity between `7` and `Y` under heavy rotation and noise.
...
Full dataset inference (all 500 images): **499/500 correct (99.8%)**

The held-out test accuracy (100 images never seen during training): **99.0%**
...
```
captcha_finetune/
  ProcessedCaptchas/       # 500 labeled captcha images
  output/
  ...
  train.py                 # v1: CRNN + CTC (95% accuracy)
  train_v2.py              # v2: Multi-Head Classification (99% accuracy)
...
2. **Heavy data augmentation is critical with small datasets.** 400 training images with aggressive augmentation achieved 99% accuracy without any synthetic data generation.
